def decrypt(C):
  ...
  # Générer un dictionnaire comme fait dans l'exemple.
  l = len(symboles)
  rnd.seed(1337)
  int_keys = rnd.sample(range(256), l)
  dictionary = {symboles[i]: "{:08b}".format(int_keys[i]) for i in range(l)}

  # Diviser le texte chiffré en blocs de 8 bits
  c_block = [C[i:i + 8] for i in range(0, len(C), 8)]
  dechiffre = ""
  
  # Trouver la clé correspondante pour chaque bloc
  for block in c_block:
      if block in dictionary.values():
